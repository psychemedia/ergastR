#' @title Wrapper for ergast.com JSON API
#'
#' @description Wrap calls to \code{ergast.com} JSON API and return R dataframes.
#'
#' @name ergastR
...
#'
#' *TO DO - this function is hacky in that an explicit maximum limit value is et rather than using a proper paged results handler*
...
#' Get JSON data
#'
#' *TO DO - really should have a handler that supports cacheing*
#' \code{getJSONbyURL}
#' @import RJSONIO
...
#' @param character url of the API call
...
#' @param character constructor reference code for specified constructor
#' @param character data format (json, XML)
#' @param character connection string for database; if this is not present use ergast API
#' @return dataframe containing qualifying results
#' @export
qualiResults.df=function(year=NA,
                         raceNum=NA,
                         driverRef=NA,
                         constructorRef=NA,
                         format='json',
...
resultsData.df=function(year,raceNum){
  ...
  for (i in 1:length(rrd)){
    race.results.data=rbind(race.results.data,data.frame(
      ...
      fastlapnum= ifelse(length(rrd[[i]]$FastestLap)>0,
                         getNum(rrd[[i]]$FastestLap$lap),
                         NA),
      fastlaptime= ifelse(length(rrd[[i]]$FastestLap)>0,
                          timeInS(rrd[[i]]$FastestLap$Time$time),
                          NA),
      fastlaprank=ifelse(length(rrd[[i]]$FastestLap)>0,
                         getNum(rrd[[i]]$FastestLap$rank),
                         NA)
    ))
  }
...
#' @param character URL for driver standings
